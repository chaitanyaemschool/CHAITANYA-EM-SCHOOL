...
export const navItems = [
  { label: "Home", to: "/", icon: Home, tooltip: "Return to the homepage", gradient: "linear-gradient(135deg,oklch(0.72 0.17 28),oklch(0.55 0.19 28))" },
  { label: "About School", to: "/about", icon: School, tooltip: "Our story, vision & leadership", gradient: "linear-gradient(135deg,oklch(0.62 0.16 258),oklch(0.36 0.13 258))" },
  { label: "Academics", to: "/academics", icon: BookOpen, tooltip: "Curriculum & learning stages", gradient: "linear-gradient(135deg,oklch(0.78 0.15 78),oklch(0.58 0.16 60))" },
  { label: "Facilities", to: "/facilities", icon: Building2, tooltip: "Campus, labs & amenities", gradient: "linear-gradient(135deg,oklch(0.65 0.14 200),oklch(0.42 0.12 220))" },
  { label: "Gallery", to: "/gallery", icon: ImageIcon, tooltip: "Photos from life on campus", gradient: "linear-gradient(135deg,oklch(0.72 0.16 320),oklch(0.48 0.17 300))" },
  { label: "Contact", to: "/contact", icon: PhoneCall, tooltip: "Reach us & find directions", gradient: "linear-gradient(135deg,oklch(0.66 0.15 148),oklch(0.42 0.14 148))" },
] as const;
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 leading-none">
          <div className="font-editorial truncate text-[32px] leading-none md:text-[42px]">CHAITANYA</div>
      ...
      <div className="relative rounded-[26px] bg-white/7 p-4 ring-1 ring-white/10">
        <div className="text-[10px] font-semibold uppercase tracking-[0.28em] text-background/50">Reach</div>
        <ul className="mt-4 space-y-3 text-[14px] text-background/88">
          ...
          <li>
            <a href={telHref} className="flex items-center gap-3 transition-colors hover:text-background">
              <Phone className="h-4 w-4 text-background/60" /> Call admissions
            </a>
          </li>
          <li>
            <a href={waHref} className="flex items-center gap-3 transition-colors hover:text-background">
              <WhatsAppIcon className="h-4 w-4 text-background/60" /> WhatsApp
            </a>
          </li>
          <li>
            <a href={mailHref} className="flex items-center gap-3 transition-colors hover:text-background">
      ...
      <div className="relative">
        <div>
          <div className="text-[10px] font-medium uppercase tracking-[0.28em] text-background/40">Explore</div>
          <ul className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-[14px] text-background/85">
            {navItems.map((n) => (
              <li key={n.label}>
                <Link to={n.to} className="transition-colors hover:text-background">{n.label}</Link>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="relative flex flex-col items-start gap-4">
        <div className="text-[10px] font-medium uppercase tracking-[0.28em] text-background/40">Follow</div>
        <div className="flex items-center gap-5">
        {[
          { Icon: InstagramMark, label: "Instagram", href: footer.instagram || SOCIAL.instagram },
